...
double RngStream_LogitBeta(double a, double b, RngStream rng)
{
	...
	if(a < .25)
	{
		top = 1.0/a*log(RngStream_RandU01(rng)) + log(RngStream_GA1(1 + a,rng));
	}
	else
	{
	...
	if(b < .25)
	{
		bot = 1.0/b*log(RngStream_RandU01(rng)) + log(RngStream_GA1(1 + b,rng));
	}
	else
	{
	...
	if(isinf(x) || isnan(x))
	{
		Rprintf("a = %.5lf, b = %.5lf, x is infinity!\n", a, b);
